...
constexpr uint8_t MAX_MESSAGE_LENGTH =
    _MAX(LONG_FILENAME_LENGTH, MAX_LANG_CHARSIZE * 2 * (LCD_WIDTH));
#else
...
#if PREHEAT_COUNT
preheat_t
    MarlinUI::material_preset[PREHEAT_COUNT]; // Initialized by settings.load()
...
#define _PLBL(N) preheat_##N##_label,
  ...
  static PGM_P const preheat_labels[PREHEAT_COUNT] PROGMEM = {
      REPEAT_1(PREHEAT_COUNT, _PLBL)};
...
volatile int8_t encoderDiff; // Updated in update_buttons, added to
                             // encoderPosition every LCD update
#endif
...
const char *MarlinUI::scrolled_filename(CardReader &theCard,
                                        const uint8_t maxlen, uint8_t hash,
                                        const bool doScroll) {
  ...
  if (theCard.longFilename[0]) {
    ...
    if (doScroll) {
      ...
      if (filename_scroll_hash != hash) { // If the hash changed...
        ...
        filename_scroll_max = _MAX(0, utf8_strlen(theCard.longFilename) -
                                          maxlen); // Update the scroll limit
        filename_scroll_pos = 0;                   // Reset scroll to the start
        lcd_status_update_delay = 8;               // Don't scroll right away
      }
      // Advance byte position corresponding to filename_scroll_pos char
      // position
      outstr += TERN(UTF_FILENAME_SUPPORT,
                     utf8_byte_pos_by_char_num(outstr, filename_scroll_pos),
                     filename_scroll_pos);
    }
#else
    theCard.longFilename[TERN(
        UTF_FILENAME_SUPPORT,
        utf8_byte_pos_by_char_num(theCard.longFilename, maxlen), maxlen)] =
...
void _wrap_string(uint8_t &col, uint8_t &row, const char *const string,
                  read_byte_cb_t cb_read_byte, bool wordwrap /*=false*/) {
  SETCURSOR(col, row);
  if (!string)
    return;
  ...
  auto _newline = [&col, &row] {
    col = 0;
    row++;             // Move col to string len (plus space)
  ...
  if (wordwrap) {
    ...
    for (;;) {
      if (!wrd)
        wrd = p; // Get word start /before/ advancing
      ...
      if (eol || ch == ' ' || ch == '-' || ch == '+' || ch == '.') {
        ...
        if (col + c > LCD_WIDTH && col >= (LCD_WIDTH) / 4)
          _newline(); // should it wrap?
        c += !eol;    // +1 so the space will be printed
        col += c;     // advance col to new position
        while (c) {   // character countdown
          --c;        // count down to zero
          wrd =
              get_utf8_value_cb(wrd, cb_read_byte, &ch); // get characters again
...
void MarlinUI::draw_select_screen_prompt(PGM_P const pref,
                                         const char *const string /*=nullptr*/,
                                         PGM_P const suff /*=nullptr*/) {
  const uint8_t plen = utf8_strlen_P(pref),
                slen = suff ? utf8_strlen_P(suff) : 0;
  uint8_t col = 0, row = 0;
...
#define ADC_MIN_KEY_DELAY 100
  if (keypad_buttons) {
...
#if HAS_LCD_MENU
    if (encoderDirection ==
        -(ENCODERBASE)) { // HAS_ADC_BUTTONS forces REVERSE_MENU_DIRECTION, so
                          // this indicates menu navigation
      if (RRK(EN_KEYPAD_UP))
        encoderPosition += ENCODER_STEPS_PER_MENU_ITEM;
      else if (RRK(EN_KEYPAD_DOWN))
        encoderPosition -= ENCODER_STEPS_PER_MENU_ITEM;
      else if (RRK(EN_KEYPAD_LEFT)) {
...
#endif
    {
      ...
      if (RRK(EN_KEYPAD_UP))
        ...
        encoderPosition = 0;
#else
      if (RRK(EN_KEYPAD_UP) || RRK(EN_KEYPAD_LEFT))
        encoderPosition -= epps;
      else if (RRK(EN_KEYPAD_DOWN) || RRK(EN_KEYPAD_RIGHT))
        encoderPosition += epps;
#endif
    }
#endif
  ...
  if (!RRK(EN_KEYPAD_F1 | EN_KEYPAD_F2 | EN_KEYPAD_F3 | EN_KEYPAD_DOWN |
           EN_KEYPAD_RIGHT | EN_KEYPAD_MIDDLE | EN_KEYPAD_UP |
           EN_KEYPAD_LEFT)) {
    ...
    if (RRK(EN_KEYPAD_MIDDLE))
      goto_screen(menu_move);

#if NONE(DELTA, Z_HOME_TO_MAX)
    if (RRK(EN_KEYPAD_F2))
      _reprapworld_keypad_move(Z_AXIS, 1);
#endif
    ...
    if (homed) {
      ...
      if (RRK(EN_KEYPAD_F3))
        _reprapworld_keypad_move(Z_AXIS, -1);
      if (RRK(EN_KEYPAD_LEFT))
        _reprapworld_keypad_move(X_AXIS, -1);
      if (RRK(EN_KEYPAD_RIGHT))
        _reprapworld_keypad_move(X_AXIS, 1);
      if (RRK(EN_KEYPAD_DOWN))
        _reprapworld_keypad_move(Y_AXIS, 1);
      if (RRK(EN_KEYPAD_UP))
        _reprapworld_keypad_move(Y_AXIS, -1);
    }
...
// If the message will blink rather than expire...
  ...
  if (ELAPSED(ms,
              progress_bar_ms + PROGRESS_BAR_MSG_TIME + PROGRESS_BAR_BAR_TIME))
    progress_bar_ms = ms;
#endif
  ...
  if (old_frm == 100) {
    if (int16_t(encoderPosition) > ENCODER_FEEDRATE_DEADZONE)
      ...
      new_frm = old_frm;
  } else if ((old_frm < 100 && new_frm > 100) ||
             (old_frm > 100 && new_frm < 100))
    new_frm = 100;
 ...
 *   - Set manual_move.axis to an axis like X_AXIS. Use ALL_AXES_ENUM for
 * diagonal moves.
 *   - Set manual_move.start_time to a point in the future (in ms) when the move
 * should be done.
 *
...
void ManualMove::task() {
  ...
  if (processing)
    return; // Prevent re-entry from idle() calls

  // Add a manual move to the queue?
  if (axis != NO_AXIS_ENUM && ELAPSED(millis(), start_time) &&
      !planner.is_full()) {

    const feedRate_t fr_mm_s = (axis <= LOGICAL_AXES)
                                   ? manual_feedrate_mm_s[axis]
    ...
    // DELTA and SCARA machines use segmented moves, which could fill the
    // planner during the call to move_to_destination. This will cause idle() to
    // be called, which can then call this function while the previous
    // invocation is being blocked. Modifications to offset shouldn't be made
    // while processing is true or the planner will get out of sync.
    processing = true;
    prepare_internal_move_to_destination(
        fr_mm_s); // will set current_position from destination
    processing = false;
    ...
    planner.buffer_line(current_position, fr_mm_s,
                        TERN_(MULTI_E_MANUAL, axis == E_AXIS ? e_index :)
                            active_extruder);

    // SERIAL_ECHOLNPAIR("Add planner.move with Axis ",
    // AS_CHAR(axis_codes[axis]), " at FR ", fr_mm_s);
...
//
void ManualMove::soon(const AxisEnum move_axis OPTARG(
    MULTI_E_MANUAL, const int8_t eindex /*=active_extruder*/)) {
  TERN_(MULTI_E_MANUAL, if (move_axis == E_AXIS) e_index = eindex);
  start_time =
      millis() + (menu_scale < 0.99f ? 0UL : 250UL); // delay for bigger moves
  axis = move_axis;
  // SERIAL_ECHOLNPAIR("Post Move with Axis ", AS_CHAR(axis_codes[axis]), "
  // soon.");
}
...
void MarlinUI::external_encoder() {
  ...
  LCDViewAction MarlinUI::lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW;
   ...
   return !BUTTON_PRESSED(
       ENC_EN); // Update encoder only when ENC_EN is not LOW (pressed)
   }
   ...
   void MarlinUI::update() {
     ...
     auto do_click = [&] {
       wait_for_unclick =
           true; //  - Set debounce flag to ignore continuous clicks
       lcd_clicked =
           !wait_for_user; //  - Keep the click if not waiting for a user-click
     ...
     if (touch_buttons) {
       ...
       } else if (!wait_for_unclick &&
                  (buttons &
                   EN_C)) // OK button, if not waiting for a debounce release:
         do_click();
     }
     ...
     if (ELAPSED(ms, next_lcd_update_ms) ||
       ...
       TERN_(HAS_SLOW_BUTTONS,
             slow_buttons =
                 read_slow_buttons()); // Buttons that take too long to read in
                                       // interrupt context
       ...
       // because encoderDiff has a non-zero residual value, making the
       // controller unresponsive. The fix clears the residual value when the
       // encoder is idle. Also check if past half the threshold to compensate
       // for missed single steps.
       static int8_t lastEncoderDiff;

       // Timeout? No decoder change since last check. 10 or 20 times per
       // second.
       if (encoderDiff == lastEncoderDiff &&
           abs_diff <=
               epps / 2)  // Same direction & size but not over a half-step?
         encoderDiff = 0; // Clear residual pulses.
       else if (WITHIN(abs_diff, epps / 2 + 1,
                       epps - 1)) { // Past half of threshold?
         abs_diff = epps;           // Treat as a full step size
         encoderDiff =
             (encoderDiff < 0 ? -1 : 1) * abs_diff; // ...in the spin direction.
       }
         ...
         if (encoderPastThreshold &&
           ...
           if (encoderRateMultiplierEnabled) {
             ...
             if (lastEncoderMovementMillis) {
               // Note that the rate is always calculated between two passes
               // through the loop and that the abs of the encoderDiff value is
               // tracked.
               const float encoderStepRate =
                   encoderMovementSteps /
                   float(ms - lastEncoderMovementMillis) * 1000;
   ...
   //#define ENCODER_RATE_MULTIPLIER_DEBUG
               ...
               SERIAL_ECHOPAIR("  ENCODER_10X_STEPS_PER_SEC: ",
                               ENCODER_10X_STEPS_PER_SEC);
               SERIAL_ECHOPAIR("  ENCODER_100X_STEPS_PER_SEC: ",
       ...
       // Instead of tracking changes just redraw the Status Screen once per
       // second.
       if (on_status_screen() && !lcd_status_update_delay--) {
         lcd_status_update_delay = TERN(HAS_MARLINUI_U8GLIB, 12, 9);
         if (max_display_update_time)
           max_display_update_time--; // Be sure never go to a very big number
         refresh(LCDVIEW_REDRAW_NOW);
       }

#if BOTH(HAS_LCD_MENU, SCROLL_LONG_FILENAMES)
       // If scrolling of long file names is enabled and we are in the sd card
       // menu, cause a refresh to occur until all the text has scrolled into
       // view.
       if (currentScreen == menu_media && !lcd_status_update_delay--) {
         lcd_status_update_delay = ++filename_scroll_pos >= filename_scroll_max
                                       ? 12
                                       : 4; // Long delay at end and start
       ...
       if ((should_draw() || drawing_screen) &&
         ...
         if (!drawing_screen)
           ...
           case LCDVIEW_CALL_REDRAW_NEXT:
             refresh(LCDVIEW_REDRAW_NOW);
           case LCDVIEW_REDRAW_NOW: // set above, or by a handler through
         ...
         const bool in_status = on_status_screen(), do_u8g_loop = !in_status;
         ...
         if (do_u8g_loop) {
           ...
           // The screen handler can clear drawing_screen for an action that
           // changes the screen. If still drawing and there's another page,
           // update max-time and return now. The nextPage will already be set
           // up on the next call.
...
#define ADC_BUTTONS_VALUE_SCALE                                                \
  1.0 // for the power voltage equal to the reference voltage
#endif
#ifndef ADC_BUTTONS_R_PULLUP
#define ADC_BUTTONS_R_PULLUP                                                   \
  4.7 // common pull-up resistor in the voltage divider
#endif
#ifndef ADC_BUTTONS_LEFT_R_PULLDOWN
#define ADC_BUTTONS_LEFT_R_PULLDOWN                                            \
  0.47 // pull-down resistor for LEFT button voltage divider
#endif
#ifndef ADC_BUTTONS_RIGHT_R_PULLDOWN
#define ADC_BUTTONS_RIGHT_R_PULLDOWN                                           \
  4.7 // pull-down resistor for RIGHT button voltage divider
#endif
#ifndef ADC_BUTTONS_UP_R_PULLDOWN
#define ADC_BUTTONS_UP_R_PULLDOWN                                              \
  1.0 // pull-down resistor for UP button voltage divider
#endif
#ifndef ADC_BUTTONS_DOWN_R_PULLDOWN
#define ADC_BUTTONS_DOWN_R_PULLDOWN                                            \
  10.0 // pull-down resistor for DOWN button voltage divider
#endif
#ifndef ADC_BUTTONS_MIDDLE_R_PULLDOWN
#define ADC_BUTTONS_MIDDLE_R_PULLDOWN                                          \
  2.2 // pull-down resistor for MIDDLE button voltage divider
#endif

   // Calculate the ADC value for the voltage divider with specified pull-down
   // resistor value
#define ADC_BUTTON_VALUE(r)                                                    \
  int(HAL_ADC_RANGE * (ADC_BUTTONS_VALUE_SCALE)*r / (r + ADC_BUTTONS_R_PULLUP))
   ...
   //
       ...
       buttons =
           (newbutton | TERN0(HAS_SLOW_BUTTONS, slow_buttons)
       ...
       if (keypad_buttons == 0) {
         ...
         if (WITHIN(b, 1, 8))
           keypad_buttons = _BV(b - 1);
       }
#endif

#if HAS_SHIFT_ENCODER
       /**
        * Set up Rotary Encoder bit values (for two pin encoders to indicate
        * movement). These values are independent of which pins are used for
        * EN_A / EN_B indications. The rotary encoder part is also independent
        * of the LCD chipset.
        */
       uint8_t val = 0;
    ...
    encoderDiff -= encoderDirection;                                           \
     ...
     if (enc != lastEncoderBits) {
       ...
       case ENCODER_PHASE_0:
         ENCODER_SPIN(ENCODER_PHASE_3, ENCODER_PHASE_1);
         break;
       case ENCODER_PHASE_1:
         ENCODER_SPIN(ENCODER_PHASE_0, ENCODER_PHASE_2);
         break;
       case ENCODER_PHASE_2:
         ENCODER_SPIN(ENCODER_PHASE_1, ENCODER_PHASE_3);
         break;
       case ENCODER_PHASE_3:
         ENCODER_SPIN(ENCODER_PHASE_2, ENCODER_PHASE_0);
         break;
       }
...
#if SERVICE_INTERVAL_1 > 0
     else if (print_job_timer.needsService(1))
       msg = service1;
#endif
#if SERVICE_INTERVAL_2 > 0
     else if (print_job_timer.needsService(2))
       msg = service2;
#endif
#if SERVICE_INTERVAL_3 > 0
     else if (print_job_timer.needsService(3))
       msg = service3;
#endif
...
#include "../gcode/queue.h"
   ...
   void MarlinUI::pause_print() {
     ...
     TERN_(HOST_PROMPT_SUPPORT,
           host_prompt_open(PROMPT_PAUSE_RESUME, PSTR("UI Pause"),
                            PSTR("Resume")));
     ...
     pause_show_message(
         ...
         PAUSE_MODE_PAUSE_PRINT); // Show message immediately to let user know
                                  // about pause in progress
...
#if ENABLED(SDSUPPORT)
             ?: TERN(HAS_PRINT_PROGRESS_PERMYRIAD, card.permyriadDone(),
                     card.percentDone())
#endif
     );
   }
   ...
   //  - On menu screens, right side (last 3 cols) acts like a scroll - half up
   //  => prev page, half down = next page
   //  - On select screens (and others) touch the Right Half for +, Left Half
   //  for -
   //  - On edit screens, touch Up Half for -,  Bottom Half to +
   //
   void MarlinUI::screen_click(const uint8_t row, const uint8_t col,
                               const uint8_t, const uint8_t) {
     const millis_t now = millis();
     if (PENDING(now, next_button_update_ms))
       return;
     ...
     if (on_edit_screen)
       ...
       if (col > (LCD_WIDTH)-5)
         // 2 * LCD_HEIGHT to scroll to bottom of next page. (LCD_HEIGHT would
         // only go 1 item down.)
         encoderDiff =
             epps * (encoderLine - encoderTopLine + 2 * (LCD_HEIGHT)) * ydir;
       else
...
#include "extui/ui_api.h"
   ...
   void MarlinUI::media_changed(const uint8_t old_status,
     ...
     TERN_(HAS_WIRED_LCD,
           next_lcd_update_ms =
               ms + LCD_UPDATE_INTERVAL); // Delay LCD update for SD activity
   ...
   void MarlinUI::eeprom_alert(const uint8_t msgid) {
     ...
     goto_screen([] {
       ...
       MenuItem_confirm::select_screen(
           GET_TEXT(MSG_BUTTON_RESET), GET_TEXT(MSG_BUTTON_IGNORE), init_eeprom,
           return_to_status, eeprom_err(editable.uint8), msg, PSTR("?"));
     });
#else
